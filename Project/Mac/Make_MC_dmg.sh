...
if [ "$KIND" = "GUI" ]; then
    ...
    for FRAMEWORK in $(ls "${FILES}/${APPNAME}.app/Contents/Frameworks" | grep framework | sed 's/\.framework//') ; do
        pushd "${FILES}/${APPNAME}.app/Contents/Frameworks/${FRAMEWORK}.framework"
            ...
            plutil -replace CFBundleIdentifier -string "net.mediaarea.${APPNAME_lower}.mac-${KIND_lower}" Resources/Info.plist
        popd

        codesign --deep --options=runtime --force --verbose --sign "3rd Party Mac Developer Application: ${SIGNATURE}" -i "net.mediaarea.${APPNAME_lower}.mac-${KIND_lower}" "${FILES}/${APPNAME}.app/Contents/Frameworks/${FRAMEWORK}.framework/Versions/5/${FRAMEWORK}"
    done

    find "${FILES}/${APPNAME}.app/Contents/PlugIns" -name "*.dylib" -exec codesign --deep --options=runtime --force --verbose --sign "3rd Party Mac Developer Application: $SIGNATURE" -i "net.mediaarea.${APPNAME_lower}.mac-${KIND_lower}" '{}' \;
